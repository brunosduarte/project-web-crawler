...
describe("Scrapper", () => {
  ...
  it("should handle errors when closing the browser fails", async () => {
    ...
    await expect(scrapper.end()).rejects.toThrowError(
  ...
  it("should handle error when launching the browser fails", async () => {
    const scrapper = new Scrapper();
    scrapper.getBrowser = jest
      .fn()
      .mockRejectedValue(new Error("Failed to launch browser"));
    await expect(
      scrapper.scrap("https://www.example.com")
    ).rejects.toThrowError("Failed to launch browser");
    await scrapper.end();
  });
